...
public class DefaultScreenRegistry implements ScreenRegistry {
    ...
    private final static String DEFAULT_DESCRIPTOR_PATH = "screens.xml";
    ...
    private void validateScreen(ScreenDefinition screen) throws ScreenRegistryException {
        ...
        if (screen.getCallback() != null && screen.getCallback().getHandlerClass() == null) {
            throw new ScreenRegistryException(String.format("Callback handler must have valid handler class [screen: %s]",
                    screen.getId()));
        }
